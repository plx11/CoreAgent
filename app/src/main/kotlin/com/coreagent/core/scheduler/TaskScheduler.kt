...
class TaskScheduler(
    ...
    suspend fun executeFullTaskQueue(workspaceId: String, memoryDao: MemoryDao) {
        while (true) {
            val nextStep = memoryDao.getNextPendingStep(workspaceId) ?: break
            try {
                runStep(workspaceId, nextStep.stepId, memoryDao)
            } catch (e: Exception) {
        ...
        val refinerClient = agentRouter.getClientForRole(AgentRole.REFINER)
        val reportPrompt = "基於以下成品，請整合並輸出全局總結報告:\n${artifacts.joinToString("\n") { it.content }}"
        val finalReport = refinerClient.sendMessage(reportPrompt)
        
        memoryDao.insertArtifact(ProducedArtifactEntity(
            workspaceId = workspaceId,
            stepId = "FINAL_REPORT",
            artifactName = "GlobalReport",
            content = finalReport
        ))
    }
    ...
    suspend fun runStep(workspaceId: String, stepId: String, memoryDao: MemoryDao) {
        try {
            ...
            if (start != -1 && end != -1 && end > start) {
                try {
                    val jsonElement = Json.parseToJsonElement(currentOutput.substring(start, end + 1))
                    if (jsonElement is JsonObject && jsonElement.containsKey("tool_call")) {
                        ...
                        val toolResult = agentService.executeTool(toolCall.toolName, toolCall.arguments)
            ...
            memoryDao.updateStepStatus(stepId, "FAILED")
            throw e
        }
    }
}
